...
def generate_random_files(start, end, step, output_dir="dataPool"):
...
# Executar bubble_sort_pool.py
def run_bubble_sort_pool(file_name):
    start_time = time.time()
    subprocess.run(['python3', 'bubble_sort_pool.py', file_name])
    end_time = time.time()
...
for n in n_values:
    ...
    time_pool = run_bubble_sort_pool(file_name)
...
with open('times_pool.txt', 'w') as file:

    file.write("N\tBubbleSort_1P\tBubbleSort_3P\n")

    for i, n in enumerate(n_values):
        file.write(f"{n}\t{times_1p[i]:.4f}\t{times_3p[i]:.4f}\t{times_pool[i]:.4f}\n")


print("Resultats guardats a times_pool.txt")
