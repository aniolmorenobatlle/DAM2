...
def run_fibonacci_3p(n):
    # Començar temporitzador
    start_time = time.time()
    
    # Executar la funció fibonacci_2p.py
    subprocess.run(['python3', 'fibonacci_3p.py', str(n)])
...
with open("times_3p.txt", 'w') as f:
    for n, t1, t2, t3 in zip(n_values, times_1p, times_2p, times_3p):
        f.write(f"{n},{t1},{t2},{t3}\n")
